...
#within the range of an obstacle, or moving out of the bounds of the area.
...
#This function finds the closest point to p. This is the LPM 
#for the RDT algorithm. After a random node is generated, it is connected
...
if __name__ == "__main__":
    ...
    while COUNTER < 1:
        ...
        for berry in berries:
            ...
            print "part 1"
            #find the rdt tree along with the destination node
            rdt = RDT(tempN.getPoint(),berryPos,0.2,stems)
            #if false is returned it means no path can be generated
            if rdt == False:
            ...
            x = []
            y = []
            # Deterime the angles for the robot at each node    
            Q = RobotMotion2(path,robot1,stems)
            #record ending time
            endTime = timeit.default_timer()
            timetaken.append(endTime-startTime)
            #if the path is incomplete it doesnt work. Continue to the next berry.
            ...
            startTime = timeit.default_timer()
            ################################
            #The following implements RDT's for the return trip#
            ################################  
